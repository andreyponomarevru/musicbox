...
import { ReleaseExtended } from "./ReleaseExtended";
...
export async function find(catNo: unknown) {
  ...
  try {
    ...
    const { rows } = await pool.query(findReleaseCatNoQuery);
    return rows;
  } catch (err) {
    logger.error(
...
export async function create(metadata: unknown) {
  const validMeta: ReleaseExtendedMeta = await schemaCreateRelease.validateAsync(
    metadata,
  );
  const release = new ReleaseExtended(validMeta);
...
export async function read(id: unknown) {
  ...
  try {
    ...
    const releaseMeta: ReleaseExtendedMeta = (
      await pool.query(getReleaseTextQuery)
    ).rows[0];

    const release = releaseMeta ? new ReleaseExtended(releaseMeta) : null;
...
export async function readByReleaseId(releaseId: unknown) {
  ...
  try {
    const getTracksTextQuery = {
      text:
        'SELECT * FROM view_track_short WHERE "releaseId"=$1 ORDER BY "trackNo", "diskNo";',
      values: [validatedReleaseId],
    };
    const tracksMeta = (await pool.query(getTracksTextQuery)).rows;

    const tracks =
      tracksMeta.length > 0 ? tracksMeta.map((row) => new Track(row)) : [];
    logger.debug(`filePath: ${__filename} \n${util.inspect(tracks)}`);
    return { tracks };
  } catch (err) {
...
export async function readAll(params: unknown) {
  ...
  try {
    ...
    const collection: ReleaseCollection = (await pool.query(readReleasesQuery))
      .rows[0];

    const releases = collection.releases
      ? collection.releases.map((row) => new Release(row))
      : [];
...
export async function update(metadata: unknown) {
  const validatedMetadata: ReleaseExtendedMeta = await schemaUpdateRelease.validateAsync(
    metadata,
  );
  const release = new ReleaseExtended(validatedMetadata);
