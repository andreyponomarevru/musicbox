...
function createTransports(env = "development") {
  // If we're in development environment, output logs to console,
  // otherwise output logs to files
  if (env === "production") {
    return [errorFileTransport, infoFileTransport];
  } else {
...
const logger = createLogger({
  ...
  existOnError: false, // do not exit on uncaughtException
});
