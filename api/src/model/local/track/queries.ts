...
export async function create(metadata: unknown) {
  const validatedMetadata: TrackExtendedMetadata = await schemaImportLocalTrack.validateAsync(
    metadata,
  );
