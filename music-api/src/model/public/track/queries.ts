...
import { TrackExtended } from "./track-extended";
import { connectDB } from "../../postgres";
import { TrackExtendedMeta, TrackMeta } from "../../../types";
...
export async function read(id: unknown): Promise<TrackExtended | null> {
  ...
  try {
    ...
    const trackMetadata: TrackExtendedMeta = (
      await pool.query(getTrackTextQuery)
    ).rows[0];
...
export async function getFilePath(id: unknown): Promise<any> {
  ...
  try {
    const getFilePath = {
      text: 'SELECT "filePath" FROM view_track WHERE "trackId"=$1',
      values: [validatedId],
    };
    const filePath = (await pool.query(getFilePath)).rows[0];
